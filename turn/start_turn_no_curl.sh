...
# Скрипт запуска TURN сервера БЕЗ curl
# Использует только встроенные инструменты

echo "=== TURN Server Startup Script (No curl) ==="
echo "Timestamp: $(date)"
...
if [ -n "$EXTERNAL_IP" ] && [ "$EXTERNAL_IP" != "0.0.0.0" ]; then
    ...
    log "TURN server will auto-detect external IP"
